...
// for future:
Command_item::Command_item(Command_function command_function,char* buffer_in) {
    this->command = command_function;
    strcpy(this->argument_buffer,buffer_in);
}
...
Knowledge_tree_ref_non_leaf::Knowledge_tree_ref_non_leaf(char* discriminating_question,
    Knowledge_tree_ref* yes_branch, Knowledge_tree_ref* no_branch) 
{
...
// deprecated: 
Knowledge_tree_ref* Knowledge_tree_ref_leaf::rearrange_knowledge_tree(Str_list* yes_no_list,
    char* new_discriminating_question,char* answer_to_new_discriminating_question, char* new_animal_name) 
{
...
void Knowledge_tree_ref::rearrange_knowledge_tree_ref(Str_list* yes_no_list,
        ...
        char* answer_to_new_discriminating_question, char* new_animal_name,Knowledge_tree_ref** outTree) {
            ...
            if ((down_casted_tree=dynamic_cast<Knowledge_tree_ref_leaf*>(*outTree))!=NULL) {
                ...
                if (strcmp("no",answer_to_new_discriminating_question)==0) {
                    Knowledge_tree_ref_non_leaf* new_node = new Knowledge_tree_ref_non_leaf(new_discriminating_question,
                        new Knowledge_tree_ref_leaf(old_animal),new Knowledge_tree_ref_leaf(new_animal_name));
                    delete(*outTree);
                    *outTree = new_node;
                }
                else if (strcmp("yes",answer_to_new_discriminating_question)==0) {
                    Knowledge_tree_ref_non_leaf* new_node = new Knowledge_tree_ref_non_leaf(new_discriminating_question,
                        new Knowledge_tree_ref_leaf(new_animal_name),new Knowledge_tree_ref_leaf(old_animal));
                    delete(*outTree);
                    *outTree = new_node;
                } else {
                    std::cerr << concatenate_strings(2,"the string passed must be 'yes' or 'no' while it's value is ",answer_to_new_discriminating_question) << endl;
            ...
            } else
            {
                if (strcmp("no",yes_no_list->element)==0) {
                    rearrange_knowledge_tree_ref(yes_no_list->next,
                        new_discriminating_question,answer_to_new_discriminating_question,new_animal_name,(&(*outTree)->no_branch));
                } else if (strcmp("yes",yes_no_list->element)==0) {
                    rearrange_knowledge_tree_ref(yes_no_list->next,
                        new_discriminating_question,answer_to_new_discriminating_question,new_animal_name,(&(*outTree)->yes_branch));
                } else {
                    std::cerr << concatenate_strings(2,"the string passed must be 'yes' or 'no' while it's value is ",answer_to_new_discriminating_question) << endl;
                    throw std::runtime_error("parmeter error");
                }
...
Knowledge_tree_ref* Knowledge_tree_ref_non_leaf::rearrange_knowledge_tree(Str_list* yes_no_list,
    char* new_discriminating_question,char* answer_to_new_discriminating_question, char* new_animal_name) 
{
    if (!(strcmp(answer_to_new_discriminating_question,"no")==0)&&!(strcmp(answer_to_new_discriminating_question,"yes")==0))
    {
...
Model_ref::Model_ref() {
    ...
    strcpy(message_from_engine_buffer_version,WELCOME_MESSAGE);
    yes_no_list=NULL;
...
char* Model_ref::get_message_from_engine() {
    return message_from_engine_buffer_version;
}

void Model_ref::set_message_from_engine(int num, ...) {
    strcpy(message_from_engine_buffer_version,"");
    va_list valist;

    va_start(valist,num);
    for (int i = 0; i<num; i++) {
        strcat(message_from_engine_buffer_version,(va_arg(valist,char*)));
    }
    va_end(valist); 
}

void Model_ref::set_message_from_engine(char* message) {
    strcpy(message_from_engine_buffer_version,message);
}
...
void Model_ref::update(char* user_input) {
    ...
    switch (state) {
        case THINK_ABOUT_AN_ANIMAL_STATE:
            set_message_from_engine((char*)THINK_ABOUT_AN_ANIMAL_MESSAGE);
            current_node_ref = knowledge_tree_ref;
            state=GUESSING_STATE;
            break;

        case GUESSING_STATE:
            set_message_from_engine(current_node_ref->get_question());
            state = current_node_ref->select_specific_checking_guess_state();
            break;

        case CHECKING_GUESS_IN_LEAF_NODE_STATE:
            if (strcmp("yes",user_input)==0) {
                set_message_from_engine((char*)"yeah");
                state=THINK_ABOUT_AN_ANIMAL_STATE;
            } else if (strcmp("no",user_input)==0) {
                set_message_from_engine("what animal was?");
        ...
        case GETTING_ANIMAL_NAME_STATE:
            ...
            set_message_from_engine(5,"what is the question to distinguish a ",
        ...
        case GETTING_ANSWER_TO_DISCRIMINATING_QUESTION:
            ...
            current_node_ref = knowledge_tree_ref;
            set_message_from_engine((char *) "let's start again. Think about an animal.");
            free_str_list(&yes_no_list);
            yes_no_list=NULL;
            state = GUESSING_STATE;
            break;
        ...
        case GETTING_DISCRIMINATING_QUESTION:
            ...
            set_message_from_engine(7,"what is the answer to the question '",
...
int main(int argc, char **argv) {
    ...
    while(1) {
        std::cout << model_ref->get_message_from_engine() << endl;
